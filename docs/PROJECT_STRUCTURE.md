...
└── src/
    ...
    │       └── graphql/
    │           └── schema.graphqls               # root Query; domain schema added in Phase 2
    │
    └── test/
        ├── java/com/apex/ledger/
...
|---|---|---|
| Double-spend / concurrent debit | `application/service` + `infrastructure/concurrency` | PostgreSQL transactional advisory lock on account id, taken inside the posting transaction and released by the DB on commit **or** crash |
| Duplicate client submission | `infrastructure/concurrency` (`IdempotencyStore`) | Redis `SET NX` on the client idempotency key, plus a unique constraint on the journal as the authoritative backstop |
